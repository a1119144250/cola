...
public class UserServiceTest extends UserBaseTest {
    ...
    public void testCache() {
        UserOperatorResponse response = userService.registerAdmin("17691020550",
                                                                  "wangjin521");
    ...
    public void testCache1() {
        ...
        Assert.assertEquals(user.getState(),
                            UserStateEnum.INIT);
        ...
        Boolean res = userService.auth(request)
                                 .getSuccess();
    ...
    public void testAuth() {
        ...
        Assert.assertEquals(user.getState(),
                            UserStateEnum.AUTH);
        Assert.assertEquals(user.getRealName(),
                            "wang");
        Assert.assertEquals(user.getIdCardNo(),
                            "1234");
    ...
    public void testActive() {
        ...
        Boolean activeResult = userService.active(userActiveRequest)
                                          .getSuccess();
        Assert.assertTrue(activeResult);
        user = userMapper.findById(user.getId());
        Assert.assertNotNull(user);
        Assert.assertEquals(user.getBlockChainUrl(),
                            "url");
        Assert.assertEquals(user.getState(),
    ...
    public void testModify() {
        ...
        UserModifyRequest userModifyRequest = new UserModifyRequest();
        userModifyRequest.setUserId(user.getId());
        String nickName = UUID.randomUUID()
                              .toString()
                              .substring(0,
                                         13);
        ...
        user = userMapper.findById(user.getId());
        Assert.assertNotNull(user);
        Assert.assertEquals(user.getProfilePhotoUrl(),
                            "xxx");
    ...
    public void testFreezeANdUnFreeze() {
        ...
        Assert.assertEquals(user.getState(),
                            UserStateEnum.ACTIVE);
        Boolean freezeResult = userService.freeze(user.getId())
                                          .getSuccess();
        Assert.assertTrue(freezeResult);
        user = userMapper.findById(user.getId());
        Assert.assertNotNull(user);
        Assert.assertEquals(user.getState(),
                            UserStateEnum.FROZEN);
        Boolean unFreezeResult = userService.unfreeze(user.getId())
                                            .getSuccess();
